...
angular.module('myApp').controller('ReadyToDeliverController', ['$scope', '$cookies', '$location',  '$route', '$rootScope','$window', '$timeout', '$interval' , '$http' , function($scope, $cookies, $location, $route, $rootScope,$window,$timeout,$interval,$http) {
    //alert("hiiii");
    var self = this;
    $scope.orders=[];
    //alert("Hiiiii");
    ...
    $scope.postcombination = {"deliveryType":"YMM Delivery","status":3}

    $http.get(serverurl + "/orders/getOrderDetailsByDeliveryPersonId/3", {
            headers: {
        ...
        $scope.chkShow= function(order) {
		   for( var i=0;i<order.orderDetails.length;i++){
		    if( order.orderDetails[i].status == 3 && order.orderDetails[i].deliveryAgentId !=0 && order.orderDetails[i].deliveryAgentId == $scope.deliverpersonid){
		          return true;  
		        }     
		    }
		    return false;        
...
// }
    ...
    $scope.deliverOrder = function(orderdetails, order) {
      var ordersend={};
      var orderDetails = []
      orderDetails.push({
        "id": orderdetails.id,
      ...
      ordersend.id = order.id;
      ordersend.customerId = order.customerId;
      ordersend.paymentMode = order.paymentMode;
      ordersend.orderDetails = orderDetails;
      console.log(ordersend);
      var orderdetailsid = orderdetails.id;
       $http.post(serverurl + "/orders/orderDelivered",ordersend, {
            headers: {
                'Authorization': tokenCookie
            }
        })
                    .then(
                        function successCallback(response) {
                          if(response.status==200){
                            $.alert.open("order delivered successfully.");
